...
def train_model(model, trainLoader, valLoader, optimizer, loss_function, device):
    ...
    for step in range(args["NUM_STEPS"]):
        ...
        if ((step % args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"] - 1)) and (step != 0):
            savePath = args["CODE_DIRECTORY"] + "/audio_only_pretrain_checkpoints/models/train-step_{:04d}-wer_{:.3f}.pt".format(
                step,
                validationWER)
            ...
            plt.legend()
            plt.savefig(
                args["CODE_DIRECTORY"] + "/audio_only_pretrain_checkpoints/plots/train-step_{:04d}-loss.png".format(step))
            plt.close()
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/audio_only_pretrain_checkpoints/plots/train-step_{:04d}-wer.png".format(step))
            plt.close()
